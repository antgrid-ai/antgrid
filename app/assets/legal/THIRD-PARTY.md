...
Antgrid's first-party distributed code is primarily MPL-2.0, with `relay/` and
`web/` under scoped Elastic-2.0 licences. [`LICENSING.md`](LICENSING.md) maps
the boundary. This file records the separate licences of third-party software
Antgrid vendors, bundles, links against, or otherwise redistributes.
...
a pointer to upstream is the whole obligation.
...
None restricts distribution under the repository's licence map. The attribution obligations that do apply —
...
copyright above is upstream's and stays, and the fork remains MIT throughout;
Antgrid's relicensing does not alter those upstream terms.
...
All three shared packages are MPL-2.0. Independently distributed packages carry
their own canonical `LICENSE` copy; [`LICENSING.md`](LICENSING.md) is the full map.

| Package | Language | Licence | Role |
|---|---|---|---|
| `packages/antgrid-wire` | TypeScript | MPL-2.0 | Shared route-frame codec and relay control-envelope schemas |
| `packages/antgrid_relay_client` | Dart | MPL-2.0 | Pure Dart relay and crypto client |
| `packages/antgrid_eval_client` | Dart | MPL-2.0 | CLI wrapper around the above, for end-to-end evals |

`antgrid_eval_client` depends on `antgrid_relay_client`, and `bridge/` and
`relay/` depend on `antgrid-wire`. The ELv2 services may distribute the MPL
wire package as part of a Larger Work; changes to the MPL files stay MPL.
...
platform icons. They are first-party reserved assets, not MPL-covered files.
Redistribution is governed by [`BRAND-ASSETS-LICENSE.md`](BRAND-ASSETS-LICENSE.md)
and [`TRADEMARK.md`](TRADEMARK.md), not by this file.
...
| `evals/` | `antgrid-wire` | workspace | MPL-2.0 | first-party |
