...
class StorageService {
  ...
  Future<DeviceIdentity?> loadIdentity() async {
    final deviceId = await _storage.read(key: scopedStorageKey('device_id'));
    if (deviceId == null) return null;

    final name =
        await _storage.read(key: scopedStorageKey('device_name')) ??
        'Antgrid App';
  ...
  Future<void> saveIdentity(DeviceIdentity identity) async {
    await _storage.write(
      key: scopedStorageKey('device_id'),
      value: identity.deviceId,
    );
    await _storage.write(
      key: scopedStorageKey('device_name'),
      value: identity.name,
    );
  ...
  Future<Uint8List?> _readBytes(String key) async {
    final value = await _storage.read(key: scopedStorageKey(key));
