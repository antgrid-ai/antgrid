...
void main() {
    ...
    final mock = MockClient((req) async {
      ...
      return http.Response(
        jsonEncode({
          ...
          'knownProjects': [
            {'projectId': 'p1', 'label': 'Proj', 'path': '/x', 'running': true},
          ],
        }),
        200,
      );
  ...
  test('phonesUnpair posts the expected verb', () async {
    ...
    final mock = MockClient((req) async {
      ...
      return http.Response(
        jsonEncode({'id': seen['id'], 'ok': true, 'type': 'phones:unpair'}),
        200,
      );
    });
    final c = HostControlClient(port: 1, token: 't', httpClient: mock);
  ...
  test(
    'phonesList throws HostControlException on a malformed phone entry',
    () async {
      final mock = MockClient((req) async {
        final body = jsonDecode(req.body) as Map<String, dynamic>;
        return http.Response(
  ...
  test(
    'remoteAccessSet sends the requested value and parses what landed',
    () async {
      var seen = <String, dynamic>{};
      final client = HostControlClient(
        port: 1,
  ...
  test(
    'a response without `enabled` parses as disabled, never as enabled',
    () async {
