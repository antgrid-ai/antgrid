...
// (RelayService.licenseErrorStream stays silent for SUPERSEDED) is pinned at
// the package level in relay_service_reconnect_test.dart; this test proves
// the app's connection-state surface — what the workspace boot screen and
// `agentReachabilityProvider` actually read — treats a SUPERSEDED disconnect
// exactly like any other non-license disconnect, and that it can never reach
// the one UI flag that DOES mean "re-authenticate"
...
void main() {
  ...
  test(
    'a SUPERSEDED disconnect is read by agentReachabilityProvider the SAME '
    'as any other non-license disconnect, and never flips '
    'authRevokedBannerProvider',
    () async {
      ...
      // agentReachabilityProvider only reads `errorCode` specially for
      // `authenticated` + PAIR_TIMEOUT; every disconnected state — regardless
      // of code — must classify identically (never a distinct "license" bucket).
      final supersededReach = container.read(agentReachabilityProvider);
      expect(supersededReach, baseline,
          reason: 'SUPERSEDED must not carve out special (license-like) '
              'reachability handling');
      expect(supersededReach, isNot(AgentReachability.online));

      expect(RelayLicenseErrorCode.fromWire('SUPERSEDED'), isNull,
          reason: 'SUPERSEDED must never parse as a license error code — '
              'the type the app would need to render "re-activate" UI');

      expect(container.read(authRevokedBannerProvider), isFalse,
          reason: 'only a local-transport auth_revoked event flips this — a '
