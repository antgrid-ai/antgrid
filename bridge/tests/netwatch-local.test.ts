...
describe("LocalListener body capture", () => {
  ...
  it("withholds a secret-bearing body even while armed", async () => {
    const ws = await connectOwner();
    armBodyCapture(true, 60_000);
    // A human's answer to a question rides this type, and `--bodies` output is
    // printed, streamed over /netwatch, and appended to an --export file that
    // gets pasted into bug reports.
    const secret = "ANSWER-MUST-NOT-BE-CAPTURED";
    ws.send(JSON.stringify(createMessage("agent:question-resolve", {
      sessionId: "s1", questionId: "q1", answer: secret,
    })));
    ...
    ws.send(JSON.stringify({ type: "agent:question-resolve", id: "x", sessionId: "s1", questionId: secret }));
    ...
    expect(seen.every((e) => e.msgType === "agent:question-resolve" && e.bytes! > 0)).toBe(true);
    ws.close();
  });
