...
export class StreamMux {
  ...
  attach(bus: MessageBus, opts: AttachStreamOpts): StreamHandle {
    ...
    return {
      ...
      // Bypasses the bus AND `admit`: this is how the shaper itself sends a
      // catch-up screen, so gating it on `admit` would deadlock a governor
      // against the very budget it exists to relieve. Still behind
      // `mayDeliver` — the machine's remote-access switch must still bind —
      // and fire-and-forget like the bus path above: nothing here awaits a
      // catch-up screen's delivery.
      sendFrame: (msg) => {
        if (!mayDeliver()) return;
        void this.transport.sendEnvelope(streamId, msg, "control");
      },
    };
  }
