...
`peer-session-owner.ts` owns peer session establishment and peer liveness.
...
- `peer-session-owner.ts` + `peer/native-host-connection.ts` own Iroh payloads, session establishment and peer liveness; `project-streams.ts`'s `ProjectStreamRegistry` (constructed in `peer-session-owner.ts`'s constructor, `this.projectStreams`) owns the host-local project bindings themselves. The session stream's I/O is the same `StreamRecordWriter`/`StreamRecordReader` pair as every other stream — its header `type` (`session` vs `message`) is the only discriminator between a liveness record and a control-plane one. Central reconnects and presence changes cannot close or establish native sessions — a live session has no in-place rekey any more (`docs/protocol/peer-session.md` §3), so any liveness failure is a full re-dial instead. Host resume and authorization recheck are required capabilities of every `RemoteHostConnection`.
...
- `project-streams.ts` — gives each project its OWN native QUIC bidi stream (Stage A wave A4), replacing the old `stream-mux.ts` `{s, m}` session-stream envelope entirely: a project record carries no `s`/`m` wrapper, and the session stream now carries only machine control-plane frames. `ProjectStreamRegistry.attach(bus, opts)` returns `StreamHandle{detach, sendTo, deliverableTo, terminalHooks?}` — no host-minted id, because the stream itself (admitted by `projectId` off the open frame) is the binding. **`opts.mayDeliver` is the OUTBOUND authorization hook**, re-read on every bus frame, so callers controlled by a policy switch must fail closed. `stream-ready {projectId}` is BOTH the Hazard-J ready notice on the session stream (gates the app's project-stream open: too-early is refused in-band `NOT_READY`, never parked) AND the bridge's own first record on an admitted project stream, which is what makes the bind itself observable — there is no separate ack. An admitted stream that overflows or protocol-violates resets only that stream (the app reopens and resyncs); a peer proven unauthorized retires the whole connection. Tunnel (preview) traffic carries no bus frames — it rides its own per-exchange QUIC streams (`docs/protocol/peer-session.md` §1c) — so `attachStream`'s `opts.tunnels` (a `TunnelStreamServer`) and `tunnelBinding(projectId)` (a `TunnelProjectBinding`, the tunnel counterpart of `projectBinding`) are how `TunnelStreamRegistry` reaches a project's admission and outbound-authorization hooks.
