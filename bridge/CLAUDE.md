...
- `work-status.ts` — pure fold from outbound bus frames (+ the inbound turn-start/answer hooks) to PER-SESSION work status; `ProjectCore.workStatus` is only its rollup. **This is the ONLY per-session reduction** — `SessionManager` folds nothing, it stamps this one's answer onto each `session:updated` entry via the injected `sessionWorkStatusFor`, and `ProjectCore.commitWork` calls `refreshSessionWork()` when the per-session map moves. That re-emit folds straight back in, so it terminates *only* because `foldSessions` returns the SAME state for an unchanged session set — keep that discipline. **Presence of `sessionStatuses` is the app's capability signal**: `{}` means "warm, nothing running", absent means an older bridge, so never omit it for a warm core. Read state is tracked PER CLIENT (keyed by `InboundSource`) and fed by three inbound signals — `sessionFocus`, `clientFocusState`, and `clientGone` (`onPeerOffline` for the phone, `LocalListener.onOwnerDisconnected` for the desktop). Nothing persists it on either side, and `app_shell.dart` must keep skipping `unread` when writing the status cache. The reasoning behind each rule is on the declarations in the file.
  ...
  - `/handler-event` with `event:"turn_end"` → `hookTurnEnd`: the SECOND closer, and the only one that does not go through a notification. codex's `notify` argv and its Stop hook fire independently (`turnBoundaryEvents.end` names both), so an inferred turn that reaches only one of them still ends. It must never clear the notifications map — it arrives AFTER the paired `task_complete`, which is the only thing `isStaleIdleNudge` reads. Not `turn_failed`: that is claude parking for the Handler, which withholds its notification on purpose.
  - `invalidateHookObservation` → `noteHookChannelLost`: a session whose injected hooks have been written off leaves `keystrokeTurnSessions` until a `/hook-alive` ping (`noteHookChannelRestored` + `SessionManager.restoreHookObservation`) proves otherwise. That set is recomputed from the agent's STATIC spec on every session list, so the mark has to live on the state or the bridge goes on inferring starts for a session whose only closer it has just declared dead — the shape of every wedged "working" dot. **Nothing else reconsiders an invalidation**, which is why the ping has to undo it in both places.
  ...
  The submit gate has **three** halves and every one is required, because each rules out a different submit that starts no model turn — and a turn opened without one has nothing that can ever close it. A PTY delivers one keystroke per frame, so the submitting CR normally arrives alone and `isSubmitKeystroke` alone cannot tell a prompt from enter on an empty line or on a TUI menu. `hasTypedContent` (also `keystrokes.ts`) is the second: it records the line in `typedSessions`. `opensCommandLine` (same file) is the third, and it reads the character that OPENED that line — `/compact`, `/clear`, `/new`, `/model` are answered by the CLI itself, so neither turn-end channel fires for one (measured on codex: a prompt fires its `notify` argv, `/compact`/`/new`/`/status` fire nothing). Only a submit on a line recorded as a `"prompt"` opens a turn, and the submit consumes the line either way — latching the classification past its own submit is the mirror failure, a session that can never read "working" again. Which agent a session runs is `s.tool ?? defaultTool`, where `defaultTool` is folded from `agent:hello` — a `SessionEntry` carries `tool` only when it OVERRODE the project's `agent.tool`, so reading the entry alone silently opted every default-spec session out of the inference.
