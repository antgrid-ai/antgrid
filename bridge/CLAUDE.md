...
- `peer-session-owner.ts` + `peer/native-host-connection.ts` own Iroh payloads, E2E, fragmentation, scheduling, credits and host-local project bindings. Central reconnects and presence changes cannot close, rekey or establish native sessions. Host resume and authorization recheck are required capabilities of every `RemoteHostConnection`.
