...
- `tunnel-manager.ts` — maps ports→preview URLs; only emits `preview:url` for proxies with `browser: true`. An entry whose label/scheme is unchanged is not re-pushed, so anything recorded while `connState.suppressed` is ALSO tracked in `undelivered` and re-pushed on the next unsuppressed pass — reconnect re-enters `onPortsUpdate` with identical ports (via `resyncState`'s `emitCurrent()`), and the app's own pull is the other half: `PreviewService` registers a `preview:snapshot:request` hydrator per checkout, so the handler answers with `preview:snapshot` plus an `emitCurrent()` on every (re)establish — a checkout whose bundle is built after the connect-time replay has no other way to learn its ports. Preview bodies are compressed inside the tunnel message (`encodeBody` in `localhost-fetch.ts`, `bodyEncoding: "gzip-base64"`), never at the WebSocket layer — the relay carries AES-GCM ciphertext, so permessage-deflate has nothing to squeeze. **Only compress what the request's `acceptEncodings` advertised**: `bodyEncoding` reaches the app as a bare string, so an app that predates the encoding renders the gzip bytes as the body. Compatibility rides on that one field in both directions — `z.object` strips it for an old bridge, and an old app never sends it. **The bridge, not the phone, is the authority on a port's scheme.** The phone can only guess `http` for a dev server it never saw announce itself (an Aspire AppHost, a `dotnet run` — anything started outside a project terminal), so `fetchLocalhost` retries the one failure that means "this listener is TLS" and memoizes the port; `onWsOpen` reads that memo (`isTlsOnlyPort`) rather than retrying, because a failed `wss` handshake is indistinguishable from a dev server refusing the connection. **The self-signed-cert exemption applies to BOTH** — the WebSocket upstream needs its own `tls.rejectUnauthorized: false`, and without it a page that only renders once its socket connects (Blazor, Vite HMR) is a blank tab with nothing logged. **`tunnel:ws-open` carries the browser's handshake headers, and `Cookie` is the load-bearing one**: a cookie-authenticated dev server reads its session off the WebSocket request, not the page load before it, so dropping them opens an ANONYMOUS socket behind an authenticated page — which renders as "not authorized" rather than as a failure, and only on the phone, since the desktop WebView dials the port itself. The app rewrites `Origin` to the dev server's own (only it knows the proxy's port), and the bridge drops what the upstream handshake mints (`WS_HOP_BY_HOP_HEADERS`). **The browser's `Sec-WebSocket-Protocol` is forwarded, not dropped** (`splitUpstreamWsHeaders` → Bun's `protocols`): every Vite-family dev server (Astro, Nuxt, SvelteKit, plain Vite) answers an HMR upgrade ONLY when it names `vite-hmr`, and one without it is left unanswered rather than refused. The app's proxy echoes the browser's first choice on its own 101 before this socket has opened (Chromium fails the handshake outright on a 101 that answers its request with no echo), so the negotiation is optimistic — a server that picks a later entry is not corrected, only logged, and a server that echoes NOTHING is not even logged: Bun opens on any answer and reports `ws.protocol` as the first entry it offered when none came back (measured), so that half of the mismatch is invisible at every layer. Every dev server seen so far echoes the one it wants. **Never close an upstream that is still CONNECTING** — `releaseUpstream` parks it (`wsAbandoned`) and closes it once it opens. Bun aborts an unfinished handshake with a RESET, and Node hands an `upgrade` to its listeners with its own error handler removed, so a dev server that ignored the upgrade holds a socket with no error listener at all and the reset is an unhandled `read ECONNRESET` that exits the process — measured on Node 26 against Astro. **The park has exactly one documented exception and one hole, and both matter.** The exception is `WS_ABANDONED_MAX`: past the cap the oldest park IS cut mid-handshake, deliberately, because an unanswered upgrade is a leaked fd and one bounded crash risk beats unbounded growth — it logs, so a dev server that dies of ECONNRESET can be traced back to it. The hole is that the park has no TTL and the cap is per-manager: the population it exists for is a server that fires neither `open` nor `close` nor `error` (Bun raises no handshake timeout, measured), `stop()` neither drains nor clears the set, and every checkout teardown builds a fresh `TunnelManager` with a fresh budget — so parked sockets accumulate across manager lifetimes with nothing able to reap them, each one pinning the dead manager its listeners close over. The subprotocol forwarding above is what shrinks that population, by keeping HMR upgrades from being the ones nobody answers. Both strings the browser chose — the subprotocol and the path — reach Bun's `WebSocket` constructor, which THROWS a `SyntaxError` for a non-token subprotocol or a URL with a fragment; `openUpstream` is the catch, and it is not optional: nothing between the relay's message listener and `onWsOpen` catches, and `index.ts` answers an uncaught exception by shutting down the whole host.
