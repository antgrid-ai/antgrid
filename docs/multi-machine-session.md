...
**A Message the sending bridge itself refused is held and redelivered.** Losing one on the wire and never putting one on the wire are different events, and only the first is what "lossy" buys. A Message the transport rejected outright — no carrier attached, the app between the two machines away — provably did not reach the relay, so sending it later is a delivery rather than a second copy, which is what makes it safe for something with no `seq` to dedup by. The bridge therefore holds refused Messages and retries them, bounded by a count and by the task-expiry clock; past either it drops the oldest, which is where the lossiness is actually spent. A Message that *did* leave is never retried, and a silence is never read as a refusal.
...
**`open-task` ships as `antgrid_raise_task`, and the name it does not have is taken.** Implementation shipped an `antgrid_open_task` first, meaning "mark an assigned task started" (it is what emits `working`, §5.1) — a different verb that refuses anything not already assigned to the caller. The two are unrelated and both are wanted, so this one is named for what it does to the lead rather than to the task. It carries the `origin: peer` this section specifies on the task record, and it crosses as its own wire type (`session-bus:raise`) rather than a flag on the assign: the two frames mint the same record and differ only in which side ends up working it, so a bridge that did not know a flag would read a raise as an assign and take the wrong role, where an unknown type is refused instead. The half that makes it load-bearing is the refusal beside it — `report-finding` now requires a `taskId` and names this verb when one is missing, because D4a's poll-only finding was never pollable: no record holds it, so no tool lists it and no id exists to fetch it by.
...
3. On **Add**, the peer bridge **creates a new session in that project for the purpose** (D10), on the main checkout — diagnosis needs the environment as it is, not a fresh worktree — with the chosen tool and model. Its row carries `memberOf`; the lead's row gains the member. Creating the membership is also what records the carrier as that peer's **route home**: a peer bridge cannot dial the lead's machine (D7), so the app session that created it is its only way back, and noting it here is what lets a peer answer before the lead has sent it anything. The route is persisted per project and reloaded on start, because a restarted peer bridge has received nothing to relearn it from while the tasks it resumes are already assigned — the lead is waiting on a result and so has no reason to send the frame that would teach it one. A relay slot id addresses an (account device, machine) pair rather than a connection, which is what keeps a remembered route correct after both ends reconnect; one that no longer resolves simply refuses the send, and the next inbound frame rebinds it.
...
Wake is triggered by task state the bridge observes. A finding with no task cannot wake anyone; a peer that has something urgent to report outside its assigned work uses `open-task` (§4.5), which is what keeps the rule uniform. *Amended:* "poll-only" was wishful — nothing polls a finding that no task record holds, so implementation refuses one outright and names that verb instead (§4.5).
...
The relay drops frames without saying which. A transition that can be lost strands the lead; a conversational message that can be lost is by design. `seq` per task, ack, retry, duplicate-is-no-op. A Message the sender's own transport refused is the one exception and not a weakening of this: it never left, so redelivering it cannot duplicate (§3.2).
