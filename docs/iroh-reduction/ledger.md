...
| A | A10 delete chunked loopback upload, collapse `PeerLink`/`MultiStreamPeerLink`, trim A7 growth, prune tests | gate green, uncommitted | *(pending commit)* |
| A | A11 dead-code sweep and old-app capability flags (`checkoutRouting`, `pullsTree`, `terminalFramesV1`) | gate green, uncommitted | *(pending commit)* |
...
- A10: wire 129; bridge 4949 pass, 16 skip, 6 fail (the six known stale-runId fixtures), 4971 total — 5 fewer than A9, all deliberate deletions of the chunked-upload and implementation-detail tests (listed by name in `stage-A-A10-contract.md` §10.3), net of the new `file:upload-local` and `copyLocal` tests (three of those added by the adversarial review, §10.2); relay 173; relay_client 273; peer_transport 70; app 4200 pass, 2 skip; `flutter analyze` clean in app and the three packages; `check:font-tokens` clean; evals 122 pass, 5 skip, 0 fail; `test:evals:native-soak` 1 pass over the full 30 minutes (677 cycles, final RSS delta 26,234,880 bytes against the 134,217,728 bound); `test:evals:dart-terminal` 7 pass; `qualify:iroh-host`, `qualify:iroh-interop` and the relay gate pass. The wire fixture generator is unchanged, so `gen:peer-vectors` was not re-run. **A7+A10 net = +1,399 non-test, non-doc source lines against the ≤ -1,915 target: missed.** A10 alone measures -516 (843 added, 1,359 removed, after the review's `copyLocal` hardening; §0 command against `7ac22642`), against A7's +1,915. Most of what A7 added is the upload and raw tunnel stream code itself, which A10 trimmed but did not remove.
- A11: wire 129; bridge 4900 pass, 16 skip, 6 fail (the six known stale-runId fixtures), 4922 total — 50 tests of deleted code removed (listed in `stage-A-A11-contract.md` §11) and one added by the adversarial review (`project-core.test.ts`: the stream a core attaches refuses a sender with no session and gates delivery on the switch); `bunx tsc --noEmit` clean in bridge and evals; relay 172 (the retired-streams and retired-metrics tests deleted, the frame-bound and `push:deliver`-maximum tests added); web 667; relay_client 266; peer_transport 70; app 4189 pass, 2 skip; `flutter analyze` clean in app and the three packages; `check:font-tokens` clean; evals 119 pass, 5 skip, with the only failure the gate-vectors git-clean guard before the commit (125 total, two fewer than A10: `local-promotion.test.ts` and the `UPDATE_REQUIRED` row); `test:evals:dart-terminal` 7 pass; `qualify:iroh-host`, `qualify:iroh-interop` and `test:evals:iroh-relay-authorization` pass. The fixture generator changed, and `gen:peer-vectors` regenerates the fixture byte-identically. **A11 net = -1,225 non-test, non-doc source lines (283 added, 1,508 removed; §0 command against `a3fd8f72`), against the ≤ -1,000 target: met.**
  - Kept because live: `sessionBusCarrier` (the eval `LocalTestClient` and `gate-session-bus.test.ts` open non-carrier loopback owners, and nothing else names the carrier).
  - Kept because live: `relay_error_banner.dart` (`workspace_shell.dart` sets its `SESSIONS` and `LICENSE` banners; `agent:relayError` itself is deleted with its only emitter).
  - Kept because live: `DeviceRecord.x25519Pub`, `key-exchange.ts` and `push/seal.ts` (push encryption); the app stops sending `x25519Pub` to `POST /account/devices`.
  - Kept because live: `endpointInventory` (the authorization snapshot), `establishedPeers()` (push targeting), `setPeerSessionProvider` (remote-frame and bus-reach gates, `push:register`), `hydrate`/`establishmentEpoch`, `isIsolatedCheckoutKind`, and the `UPGRADE_REQUIRED` terminal version check.
  - The relay's `MAX_CONTROL_FRAME_BYTES` (64 KiB) drops an over-bound frame without a close frame, so a client sees 1006, not the 1009 the contract predicted.
...
- **Resolved (A10): native soak RSS bound.** It failed at `848df2f2`, but no leak was found in the soak's process. The bound samples `process.memoryUsage().rss` of the `bun test` process. That process holds the in-process relay, each cycle's fake license API (`Bun.serve`) and the eval `RelayClient` with its `@number0/iroh` endpoint. It does not sample the bridge child.
  ...
  - **A10 applies the first candidate:** `Bun.gc(true)` now runs immediately before the baseline `initialRss` read and before every per-cycle RSS read (`evals/soak/native-fault-soak.test.ts`). A full-duration run passes under it: 677 cycles over 30 minutes, final RSS delta 26,234,880 bytes (about 25 MiB).
