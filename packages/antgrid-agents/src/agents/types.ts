...
import type { TerminalLaunchRequest, PreparedTerminalLaunch } from "./terminal-launch";
...
export interface DriverLifecycleEvent {
  event: "limit_hit" | "limit_cleared" | "turn_failed";
  resetsAt?: number;
  errorClass?: string;
...
export interface LaunchAugmentation {
  ...
  observation?: TerminalObservationAvailability;
}

export interface TerminalObservationAvailability {
...
/**
 * Constructs the driver synchronously without acquiring a provider process.
 * Startup acquires resources only after the host owns the disposal handle;
 * a failed start must leave every acquired resource reachable by dispose().
 */
...
export interface AgentForkSupport {
  decodeLegacyArgs?: (args: readonly string[]) => string | undefined;
