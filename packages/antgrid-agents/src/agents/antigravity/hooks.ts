...
export function ensureAntigravityHook(
  ...
  assetDirectory: string = geminiConfigDir,
): boolean {
  const hooksPath = join(geminiConfigDir, "hooks.json");
  try {
    const scriptPath = antigravityScriptPath(assetDirectory);
    let data: any = {};
...
export function inject({ geminiConfigDir, abDir }: HookInjectCtx): LaunchAugmentation {
  const notificationsInjected = ensureAntigravityHook(geminiConfigDir, abDir);
  return {
    args: [],
...
export const observation = { notifications: true, titles: true, handler: false, turnStart: false, turnEnd: false, hookAlive: false } as const;
