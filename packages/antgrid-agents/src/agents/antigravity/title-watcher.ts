...
import { antigravityCliHome, parseAntigravityRenames } from "./title";

export function observeAntigravityTitles(onTitle: (nativeId: string, title: string, kind: "manual") => void): { stop(): void } {
  const watcher = new AntigravityTitleWatcher(antigravityCliHome(), (id, title) => onTitle(id, title, "manual"));
  watcher.start();
  return watcher;
}
