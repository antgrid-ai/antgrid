...
export const AntgridSessionNamer: Plugin = async () => ({
  event: async ({ event }: { event: any }) => {
    ...
    const runId = process.env.ANTGRID_RUN_ID;
    ...
    if (notifyType) {
      try {
        await fetch(`http://127.0.0.1:${port}/notify`, {
          ...
          body: JSON.stringify({ runId, type: notifyType, ...(terminalId ? { terminalId } : {}) }),
    ...
    if (type === "session.updated") {
      ...
      try {
        await fetch(`http://127.0.0.1:${port}/session-title`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ runId, terminalId, sessionId, title: title || undefined, agent: "opencode" }),
          signal: AbortSignal.timeout(BRIDGE_TIMEOUT_MS),
        });
    ...
    try {
      await fetch(`http://127.0.0.1:${port}/handler-event`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ runId, terminalId, agent: "opencode", event: handlerEvent }),
        signal: AbortSignal.timeout(BRIDGE_TIMEOUT_MS),
      });
