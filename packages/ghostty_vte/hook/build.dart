...
bool _shouldPreferSourceBuild(BuildInput input) {
  // Antgrid vendors this package only to carry a pure-Dart FFI fix
  // (VtRgbColor.fromNative; see high_level.dart) — the native libghostty-vt
  // is unchanged, so a local checkout must still DOWNLOAD the official
  // prebuilt rather than fall back to a source build (which needs a Ghostty
  // source tree we don't ship). Opt into source builds explicitly via
  // GHOSTTY_VTE_PREFER_SOURCE for upstream development.
  final override = Platform.environment['GHOSTTY_VTE_PREFER_SOURCE'];
  return override != null && override.isNotEmpty && override != '0';
}
