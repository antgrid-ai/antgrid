...
- **Git commits via the Bash tool: use `-m` flags (repeat per paragraph), never PowerShell `@'...'@` here-strings** — the Bash tool is bash, so `@'...'@` leaves a stray `@` in the subject (has shipped to remote more than once). For the same reason a message quoting an identifier in backticks needs `-F <file>`: inside a double-quoted `-m` bash substitutes the backticked word away and the commit lands with a hole where the name was.
- **Bun workspace filter names ≠ directory names**: `antgrid-bridge`/`-relay`/`-web`/`-wire`/`-evals`, not `bridge`/`relay`/etc. `bridge/integrations` breaks even that pattern — it is `@antgrid/integrations`. One root `bun install` covers every workspace — but **not** `site/` or `aspire/`, which are separate Bun projects with their own lockfiles and need their own install.
...
- **A failed Windows CMake configure poisons `app/build/` permanently.** The install-prefix block in `app/windows/CMakeLists.txt` sits *after* `include(flutter/generated_plugins.cmake)`, and `CMAKE_INSTALL_PREFIX_INITIALIZED_TO_DEFAULT` is true only on the FIRST configure — so one plugin error (a missing JDK will do it — `DEVELOPMENT.md` has the setup and the `LNK1104` symptom) leaves the prefix at `C:/Program Files/antgrid` and no later successful configure ever corrects it. Symptom is a clean compile that dies in INSTALL with "cannot create directory … Maybe need administrative privileges", which reads like a permissions problem and is not. Fix the plugin error, then delete `app/build/windows` — re-running the build alone cannot recover.
...
**Dart analysis — MCP tool while iterating, CLI to gate.** Implementers iterate
with `analyze_files`; the controller gates once with `flutter analyze`, which
cannot run concurrently (see the startup-lock gotcha). `analyze_files` reports
ERRORS ONLY — warnings and infos surface only in the CLI run, so never take an
agent's "No errors" as the gate. The MCP server is faster, not cheaper: its
analyzer is per-session and retained, where `flutter analyze` is transient.
`DEVELOPMENT.md` carries the mechanism and how to measure it.
...
- **App capabilities are hand-mirrored across the licence boundary.** The app's hello literals (`connection_handshake.dart` and `local_transport.dart` in `packages/antgrid_relay_client`) must name every key `SessionHelloCapabilities` (`bridge/src/protocol.ts`) declares, and vice versa: Zod strips an undeclared key, no suite spans both sides, and for `pullsTree` the fail direction is silent — the bridge resumes pushing every checkout's `tree:full` at each desktop reconnect, the flood `docs/architecture.md` ("Tree state flows pull-first") exists to prevent.
- **Session-bus frames route by `sessionId` on the project stream, never by checkout.** The bridge resolves the checkout from the session, so nothing in that family joins `CHECKOUT_VARIABLE_MESSAGE_TYPES`. Spec: `docs/session-messaging.md`.
