...
- **A multi-machine session's LIMITS are mirrored by hand into Dart, and drift is silent.** `MAX_SESSION_MEMBERS` (`bridge/src/protocol.ts`) and `MAX_BRIEF_CHARS` (`bridge/src/session-bus/delivery.ts`) are what actually refuse; `kMaxSessionMembers` and `kMaxBriefChars` (`app/lib/models/session_entry.dart`) exist so the UI can say the cap is reached instead of composing a request the bridge will reject. Membership and bus frames themselves route by `sessionId` on the project stream and the bridge resolves the checkout, so nothing in that family joins `CHECKOUT_VARIABLE_MESSAGE_TYPES`. Spec: `docs/multi-machine-session.md`.
