...
- **App capabilities are hand-mirrored across the licence boundary.** The app's hello literals (`connection_handshake.dart` and `local_transport.dart` in `packages/antgrid_relay_client`) must name every key `AppReadyMessage.capabilities` (`bridge/src/protocol.ts`) declares, and vice versa: Zod strips an undeclared key, no suite spans both sides, and for `pullsTree` the fail direction is silent — the bridge resumes pushing every checkout's `tree:full` at each desktop reconnect, the flood `docs/architecture.md` ("Tree state flows pull-first") exists to prevent.
