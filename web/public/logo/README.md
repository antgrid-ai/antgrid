...
| `favicon.ico` | Multi-resolution raster fallback, tiled (16 solid, 32/48 full detail). |
...
(The Hono site serves `public/logo/` at the `/logo/*` path — see `web/src/app.ts`.)

## Social card

`../og/antgrid-card.png` (1200x630) is the `og:image` for every page here, served
at `/og/*`. It is generated, not shot from a page: this service has nothing worth
photographing behind its sign-in door, so it is the lockup centred on the page
colour. The ground is opaque on purpose — several preview clients composite a
transparent PNG onto white, and paper ink would vanish there.

`ui/layout.tsx` must emit it as an ABSOLUTE URL (`ui/origin.ts`): a scraper
